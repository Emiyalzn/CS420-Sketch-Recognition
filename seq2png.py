...
from utils.svg2png import main as svg2png
import glob
...
def main():
    ...
    for category in range(len(dataset)):
        train_strokes, valid_strokes, test_strokes = load_dataset(in_dir, 'sketchrnn_'+dataset[category]+'.npz')
        ...
        for i in range(3):
            svg2png(os.path.join(svg_path, temp[i]), os.path.join(png_path, temp[i]))
